...
void doShift(char& ch, int shiftAmount, int list[]);
...
int main() {
	...
	initialize(letterCount);														//Initialize letters


	//Read through whole file, counting letters
	while (!encryptedIn.eof()) {

		characterCount(encryptedIn, ch, letterCount);								//Tally characters
		encryptedIn.get(ch);														//Grab new character
	}
	...
	while (!encryptedIn.eof()) {
		...
		encryptedIn.get(ch);														//Grab the next character
	}
...
void initialize(int list[]) {
	...
	//Set array elements to match alphabet for the list
	for (int j = 0; j < 26; j++) {
...
void copyDecryptedtext(ifstream& encryptedIn, char& ch, int list[], int shiftAmount) {
	...
	while (ch != '\n') {
		...
		doShift(ch, shiftAmount, list);										//call doShift
...
void doShift(char& ch, int shift, int list[])
{
